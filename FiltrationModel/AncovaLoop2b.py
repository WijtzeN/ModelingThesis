...
write_file = 'Ancova_Loop2_2bbbbb.xlsx'
...
# --- Variables ---
controlled_factors = ['C(Temp)']#'C(Substance)', 'C(Flow)', 'C(Gas)']
...
for _, row in df.iterrows():
  ...
  for i in range(1, 11):
    ...
    new_row["step"] = i
    new_row["Gas"] = "Steam" if i in (5, 6) else "N2"
    ...
    if i >= 6:
        new_row["P-P0"] = new_row["P_Pa"] - new_row["P_0"]
    else:
        new_row["P-P0"] = np.nan
...
summary_definitions = {
    ...
    "Temp": df_long["Temp"],
    "Flow": df_long["Flow"],
    "Gas": df_long["step"].apply(lambda s: "Steam" if s in (5, 6) else "N2"),
    "Substance": df_long["Substance"],
    "Total dust (g)": df_long["Total dust (g)"],
    "P_Pa": df_long["P_Pa"],
    "P_Pa_no_0": df_long["P_Pa"].where(df_long["Total dust (g)"] != 0, np.nan),
    "R_mem_m^-1": df_long["R_mem_m^-1"],
    "R_filter_m^-1": df_long["R_filter_m^-1"],
    "X_m.Pa": df_long["x_m.Pa"],
    "J_flat_m/s": df_long["J_flat_m/s"],
    "mu_flat_Pa.s": df_long["mu_flat_Pa.s"],
    "Temp_flat_K": df_long["Temp_flat_K"],
    "P_0": df_long["P_0"],
    "R_0": df_long["R_0"],
    "alpha_apparent": df_long["alpha_apparent"],
    "XMass_m.Pa.kg": df_long["x_m.Pa"] * df_long["Total dust (g)"] / (1000*(0.25*(0.06**2)*3.1416)),
    "X2Mass": ((df_long["mu_flat_Pa.s"] ** 2) * (df_long["J_flat_m/s"]**2) * (df_long["Total dust (g)"] / (1000*(0.25*(0.06**2)*3.1416)))),

}

summary_df = pd.DataFrame({
    name: series for name, series in summary_definitions.items()
})
